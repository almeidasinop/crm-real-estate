...
const FinancePage = () => {
  ...
  const { 
    ...
    defaultDescription: 'Acompanhe suas receitas, despesas e rentabilidade da sua operação imobiliária'
  });

  const [timeFrame, setTimeFrame] = useState('year');
  const [filterCategory, setFilterCategory] = useState('all');
  const [activeTab, setActiveTab] = useState('overview');
  const [incomeTitle, setIncomeTitle] = useState('Gestão de Receitas');
  const [incomeDescription, setIncomeDescription] = useState('Acompanhe, categorize e analise todas as suas fontes de receita imobiliária');
  const [expensesTitle, setExpensesTitle] = useState('Gestão de Despesas');
  const [expensesDescription, setExpensesDescription] = useState('Categorize e otimize todas as suas despesas relacionadas ao negócio');
  const [reportsTitle, setReportsTitle] = useState('Relatórios Financeiros');
  const [reportsDescription, setReportsDescription] = useState('Gere relatórios detalhados para analisar o desempenho financeiro do seu negócio');
  const [forecastTitle, setForecastTitle] = useState('Previsões Financeiras');
  const [forecastDescription, setForecastDescription] = useState('Simule diferentes cenários para antecipar a evolução da sua situação financeira');
  const [budgetTitle, setBudgetTitle] = useState('Gestão Orçamentária');
  const [budgetDescription, setBudgetDescription] = useState('Planeje e acompanhe seu orçamento para otimizar suas despesas');
  ...
  const handleTabChange = (value: string) => {
    ...
    toast.info("Changement d'onglet", {
      description: `Você está consultando agora a aba ${value === 'overview' ? 'Visão Geral' : 
                          value === 'income' ? 'Receitas' : 
                                                         value === 'expenses' ? 'Despesas' :
                                                         value === 'forecast' ? 'Previsões' :
                                                         value === 'budget' ? 'Orçamento' : 'Relatórios'}`
    });
  };
